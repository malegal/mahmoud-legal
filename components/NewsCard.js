...
export default function NewsCard({ news }) {
  const badgeColors = {
    'إنجاز قضائي': 'background:var(--matte-gold-light, #D4AF37); color:#000;',
    'فعالية': 'background:var(--deep-navy); color:#fff;',
    'تطوير': 'background:var(--very-dark-navy); color:#fff;',
  };
  const badgeStyle = badgeColors[news.category] || 'background:var(--matte-gold-light, #D4AF37); color:#000;';
    ...
    <div className="sector-link">
      <Link href={`/news/${news.slug}`} style={{ display: 'block', textDecoration: 'none', color: 'inherit' }}>
        <div className="experience-card" style={{ textAlign: 'right', position: 'relative' }}>
          ...
          <span style={{ color: 'var(--matte-gold-light, #D4AF37)', fontWeight: '700', fontSize: '0.8rem', marginTop: '0.5rem', display: 'inline-block' }}>
            اقرأ التفاصيل ←
          </span>
        </div>
      </Link>
