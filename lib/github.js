...
// دالة مساعدة لتحويل التاريخ إلى سلسلة نصية
...
export async function getAllArticles() {
  ...
  const sorted = validArticles.sort((a, b) => a.date.localeCompare(b.date) * -1);
...
export async function getAllNews() {
  ...
  const sorted = validNews.sort((a, b) => a.date.localeCompare(b.date) * -1);
  cache.set(cacheKey, sorted);
  return sorted;
}
